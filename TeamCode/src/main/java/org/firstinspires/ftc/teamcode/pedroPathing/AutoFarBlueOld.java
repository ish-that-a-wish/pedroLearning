...
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.GATE_PICKUP;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.GATE_PICKUP_NO_INTAKE;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.HUMAN_PLAYER_PICKUP;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.INIT_POSE;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.MOVE_OFF_LINE;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.SECRET_TUNNEL_AFTER_SHOOTING;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.SHOOTING_POSE;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveOffLaunchLine;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveToGate;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveToGateNoIntake;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveToHumanPlayer;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveToSecretTunnelAfterShooting;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveToShootAfterSecretTunnelShooting;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveToShootGate;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveToShootGateNoIntake;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveToShootHumanPlayer;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveToShootSpike1;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveToShootSpike2;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveToSpike1Pickup;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.moveToSpike2Pickup;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.multiplier;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.postSecretTunnel;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.preHumanPlayer;
import static org.firstinspires.ftc.teamcode.pedroPathing.Far.BlueFarConstants.preSecretTunnelAfterGate;
...
@Disabled
@Autonomous
public class AutoFarBlueOld extends LinearOpMode {
    ...
    public gameColor currentColor = gameColor.BLUE; // defualt to blue
            ...
            if(gamepad1.dpadUpWasReleased()) {
                if (!orderToPickup.isEmpty()) {
                    ...
                    if(lastAction == AutoAction.GATE_NO_INTAKE){
                        ...
                        gateNoIntake = waitHelper(moveToGateNoIntake, moveToShootGateNoIntake, Math.toRadians(180), gateNoIntake);
                    }
                    if (lastAction == AutoAction.FIRST_SPIKE) {
                        Log.i("ADDED WAITS: ", "FIRST SPIKE");
                        pickUpSpike1 = waitHelper(moveToSpike1Pickup, moveToShootSpike1, Math.toRadians(180), pickUpSpike1);
                    }
                    if (lastAction == AutoAction.SECOND_SPIKE) {
                        Log.i("ADDED WAITS: ", "SECOND SPIKE");
                        pickUpSpike2 = waitHelper(moveToSpike2Pickup, moveToShootSpike2, Math.toRadians(180), pickUpSpike2);
                    }
                    if (lastAction == AutoAction.GATE_PICKUP) {
                        Log.i("ADDED WAITS: ", "GATE PICKUP");
                        gatePickup = waitHelper(moveToGate, moveToShootGate, Math.toRadians(180), gatePickup);
                    }
                    if(lastAction == AutoAction.MOVE_OFF_LINE){
                        Log.i("ADDED WAITS: ", "MOVING OFF LINE");
                        moveOffLine = waitHelper(moveOffLaunchLine, Math.toRadians(180), moveOffLine);
                    }
                    if(lastAction == AutoAction.SECRET_TUNNEL && isSecretTunnelAfterGate()){
                        Log.i("ADDED WAITS: ", "SECRET TUNNEL AFTER GATE");
                        secretTunnelAfterGate = waitHelper(preSecretTunnelAfterGate, postSecretTunnel, Math.toRadians(180), secretTunnelAfterGate);
                    }
                    if(lastAction == AutoAction.SECRET_TUNNEL && !isSecretTunnelAfterGate()){
            ...
            if(gamepad1.leftBumperWasReleased()) {
                ...
                if (!orderToPickup.isEmpty()) {
                    ...
                    if(lastAction == AutoAction.GATE_NO_INTAKE){
                        ...
                        gateNoIntake = waitHelperStartWait(moveToGateNoIntake, moveToShootGateNoIntake, Math.toRadians(180), gateNoIntake);
                    }
                    if (lastAction == AutoAction.FIRST_SPIKE) {
                        Log.i("ADDED WAITS: ", "FIRST SPIKE");
                        pickUpSpike1 = waitHelperStartWait(moveToSpike1Pickup, moveToShootSpike1, Math.toRadians(180), pickUpSpike1);
                    }
                    if (lastAction == AutoAction.SECOND_SPIKE) {
                        Log.i("ADDED WAITS: ", "SECOND SPIKE");
                        pickUpSpike2 = waitHelperStartWait(moveToSpike2Pickup, moveToShootSpike2, Math.toRadians(180), pickUpSpike2);
                    }
                    if (lastAction == AutoAction.GATE_PICKUP) {
                        Log.i("ADDED WAITS: ", "GATE PICKUP");
                        gatePickup = waitHelperStartWait(moveToGate, moveToShootGate, Math.toRadians(180), gatePickup);
                    }
                    if(lastAction == AutoAction.SECRET_TUNNEL && isSecretTunnelAfterGate()){
                        Log.i("ADDED WAITS: ", "SECRET TUNNEL AFTER GATE");
                        secretTunnelAfterGate = waitHelperStartWait(preSecretTunnelAfterGate, postSecretTunnel, Math.toRadians(180), secretTunnelAfterGate);
                    }
                    if(lastAction == AutoAction.SECRET_TUNNEL && !isSecretTunnelAfterGate()){
                        Log.i("ADDED WAITS: ", "SECRET TUNNEL AFTER SHOOTING");
...
//                        pathsToAddWait.add(moveToShootHumanPlayer);
        ...
        while (opModeIsActive()) {
            Log.i("MULTIPLIER VALUE: ", String.valueOf(multiplier));
//            Log.i("WAITING METHOD CURRENT POSE: ", String.valueOf(follower.getPose()));
    ...
    private void buildPaths() {
        ...
        secretTunnelAfterGate = buildAPath(preSecretTunnelAfterGate, postSecretTunnel, Math.toRadians(180));
...
//        AutoAction currentAction = AutoAction.MOVE_OFF_LINE;
    ...
    private void addIfNotPresent(
            ArrayList<AutoFarBlueOld.AutoAction> list,
            AutoFarBlueOld.AutoAction action
    ) {
